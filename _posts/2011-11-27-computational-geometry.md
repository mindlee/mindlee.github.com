...
$$
p1\times p2 = \begin{vmatrix} x1 &x2 \\ y1 &y2 \end{vmatrix} = x1y2 - x2y1 = -p2\times p1
$$
...
如果叉积为正，则 p0p1 在 p0p2 的顺时针方向上，如果为负，则 p0p1 在 p0p2 的逆时针方向上。
...
找出一条垂直线，近似平分，左边的为 PL，右边的为 PR，类似地X数组划分为两个 XL 和 XR，分别包含 PL和 PR 中的点，按 x 坐标递增排序。类似的，Y 数组划分为连个 YL 和 YR，分别包含 PL 和 PR 中的店，按 y 坐标递增排序。
